...
print("Gene's genomic positions are being read.")
...
cons = dict()
with pyBigWig.open(gerp_bigwig_path) as bw:
...
with pyBigWig.open(gerp_bigwig_path) as bw:
    for gene in gene_pos_dict.keys():  # Iterate over the genes
        ...
        df_genes_out = df_genes_out.append({
            "query_name": gene,
            "chromosome": gene_pos[0],
            "positions": gene_pos[1],
            "scores": scores,
        }, ignore_index=True)
        if reporter == len(gene_pos_dict)-1 or reporter % 100 == 0:
            progressBarForTerminal(reporter, len(gene_pos_dict)-1)
